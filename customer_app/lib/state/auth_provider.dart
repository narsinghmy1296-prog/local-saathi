...
class AuthProvider extends ChangeNotifier {
  ...
  /// Set to true right after a forced logout from a 401, so the UI can show
  /// "session expired" once instead of just silently landing on login.
  ...
  Future<bool> login(String phone_, String password) async {
    loading = true;
    error = null;
    notifyListeners();
    try {
      await _authService.login(phone: phone_, password: password);
      name = await TokenStorage.instance.getName();
      role = await TokenStorage.instance.getRole();
      phone = await TokenStorage.instance.getPhone();
  ...
  Future<bool> register(String phone, String name_, String password) async {
    loading = true;
    error = null;
    notifyListeners();
    try {
      await _authService.register(phone: phone, name: name_, password: password);
      return true;
    } on ApiException catch (e) {
      error = e.message;
      return false;
